 ...
 * kns info
 */
export const useKnsPoolInfo = ({ account, decimals = 18 }: { account: string; decimals?: number }): knsInfoType  => {
  ...
  useEffect(() => {
    if (account) {
      ...
      contract.methods.userInfo(account).call().then(async (e: knsInfoType) => {
 ...
 * ERC20 balance
 */
export const useErc20Balance = ({ account, contract, decimals = 18 }: { account: string; contract: any; decimals?: number }): string  => {
...
export const usePoolLastTime = ({ contract }: { contract: any; }): string => {
  ...
  // const reload = useTimer()
  useEffect(() => {
    contract?.methods?.lastRewardBlock().call().then(async (e: string) => {
      console.log(e)
    })
  // eslint-disable-next-line
  }, [])
...
export const usePoolTotalStake = ({ contract }: { contract: any; }): number => {
  const [stake, setStake] = useState(0)
  useEffect(() => {
    try {
      contract?.methods?.totalStake()?.call().then(async (e: string) => {
        const _stake = weiToNumber(e, 18)
        setStake(_stake)
      })
    } catch (error) {
      console.log(error)
...
export const useKnsReward = ({ contract, account }: { contract: any; account: string }) => {
  const [reward, setReward] = useState('0')
  const reload = useTimer()
  useEffect(() => {
    if (account) {
      contract?.methods?.pendingReward(account).call().then(async (e: string) => {
