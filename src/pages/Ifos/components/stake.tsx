...
const IfoStakeView = ({ ifo, rewardValue, totalLpStakeValue }: { ifo: Ifo; rewardValue: number; totalLpStakeValue: string }) => {
  ...
  return (<IfoSatkeBoxStyle isAccount={Boolean(account)}>
    {!account ? <ConnectWalletButton width="100%" /> : (<>
      <CouponStyle>
        <div>
          <div className="row-title">YOUR LP TOKENS COMMITTED</div>
          <RowBetween>
            <div className="row-value-1">{lpStakeValue || '0.00'}</div>
            <div className="row-value-2">~{totalRetio ? totalRetio * ifo.poolBasic.raiseAmountNumber : 0} USD | {totalRetio ? (totalRetio *100).toFixed(2) : '0.00' }% of total</div>
          </RowBetween>
        </div>
        <div>
          <div className="row-title">{ ifo.sellToken.symbol } TO RECEIVE</div>
      ...
      {!hasApprove ? (
        <AutoRow justify="center">
          <Button width="100%" variant="gradual" onClick={onApprove}>Approve</Button>
        </AutoRow>
      ) : (
        <AutoRow justify="center">
            <Button width="100%" variant="gradual" disabled={!isLive} onClick={_onSupply}>Commit LP Tokens</Button>
        </AutoRow>
      )}
    </>)}
    <NumberSelect
