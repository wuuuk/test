...
const IfoProjectCard: React.FC<IfoCardProps> = ({ ifo, isVisible }) => {
  ...
  const cardWidth = isMobile ? 400 : 458
          ...
          <Card>
            ...
            >
              <Collapse.Panel header="" key={ ifo.pid } className="site-collapse-custom-panel">
                ...
                <RowBetween>
                  <Text small>End Time</Text>
                  <Text small>{dayjs(closeAt).format('YYYY-MM-DD HH:mm:ss')}</Text>
                </RowBetween>
                <RowBetween>
                  <Text small>For Sale</Text>
                  <Text small>{ifo.poolBasic.saleAmount}</Text>
                </RowBetween>
                <RowBetween>
                  <Text small>To raise(USD)</Text>
                  <Text small>{ifo.poolBasic.raiseAmount}</Text>
                </RowBetween>
                <RowBetween>
                  <Text small>Total raised(% of target)</Text>
                  <Text small>{!chainId ? '--' : `${totalCommitted}%`}</Text>
                </RowBetween>
                <RowBetween>
                  <Text small>Total burn</Text>
                  <Text small>{ifo.poolBasic.cakeToBurn}</Text>
                </RowBetween>
                <div style={{ height: '20px' }} />
                <AutoRow justify="center">
                  <LinkExternal href={ifo.projectSite} style={{ alignSelf: 'center' }}>View project site</LinkExternal>
                </AutoRow>
              </Collapse.Panel>
            </Collapse>
          </Card>
